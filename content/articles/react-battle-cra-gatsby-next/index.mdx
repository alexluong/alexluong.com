...
<CodeSandbox id="sidebar-cra-hv0rc" title="Sidebar Example with CRA" />
...
When the sidebar is opened and you refresh the page, you can see that it's closed instead. Then, when you click the toggle button, the sidebar will close instead of opening.
...
The same code here would also [work in Next](https://codesandbox.io/s/sidebar-next-working-2w679). Furthermore, let me clarify that the problem that we have encountered was not a Gatsby issue. It's an [SSR issue that Next also endures](https://codesandbox.io/s/sidebar-next-error-60smd).
...
And of course, CodeSandbox for you here. Go wild.

<CodeSandbox id="sidebar-next-best-mtb2z" title="NextJS Sidebar with best user experience" />
